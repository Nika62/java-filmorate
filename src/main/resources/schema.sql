...
(
    ...
    CONSTRAINT FILMS_GENRES_FK_1 FOREIGN KEY (GENRE_ID) REFERENCES PUBLIC.GENRES (ID) ON DELETE CASCADE ON UPDATE CASCADE
);
CREATE TABLE IF NOT EXISTS PUBLIC.USERS_LIKE_FILMS
(
    USER_ID BIGINT NOT NULL,
    FILM_ID BIGINT NOT NULL,
    CONSTRAINT USERS_LIKE_FILMS_PK PRIMARY KEY (USER_ID, FILM_ID),
    CONSTRAINT USER_LIKE_FILMS_FK FOREIGN KEY (USER_ID) REFERENCES PUBLIC.USERS (ID) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT USER_LIKE_FILMS_FK_1 FOREIGN KEY (FILM_ID) REFERENCES PUBLIC.FILMS (ID) ON DELETE CASCADE ON UPDATE CASCADE
);
CREATE TABLE IF NOT EXISTS PUBLIC.FRIENDSHIP
(
    USER_WHO_SEND_ID     BIGINT                            NOT NULL,
    USER_MUST_CONFIRM_ID BIGINT                            NOT NULL,
    STATUS               ENUM ('approved', 'not_approved') NOT NULL,
    CONSTRAINT FRIENDSHIP_PK PRIMARY KEY (USER_WHO_SEND_ID, USER_MUST_CONFIRM_ID),
    CONSTRAINT FRIENDSHIP_FK FOREIGN KEY (USER_WHO_SEND_ID) REFERENCES PUBLIC.USERS (ID) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT FRIENDSHIP_FK_1 FOREIGN KEY (USER_MUST_CONFIRM_ID) REFERENCES PUBLIC.USERS (ID) ON DELETE CASCADE ON UPDATE CASCADE
);
